...
- Ensure CLI paths that pass create/security inputs rely on service-layer validation for canonical security values (`public`, `internal`, `confidential`, `secret`).
- Ensure create operations preserve deterministic idempotency and concurrency behavior from the service layer.
