...
- For tools with create paths, add idempotency tests that cover lookup/insert/`IntegrityError`/reload behavior.
- For tools with uniqueness constraints, add concurrency tests proving deterministic conflict outcomes.
