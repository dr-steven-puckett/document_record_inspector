...
- API prompts must enforce resource-level router prefixes only (platform owns `/v1/tools/<tool-name>` mount).
- Service prompts must enforce canonical security values and deterministic idempotency/concurrency handling where create flows exist.
