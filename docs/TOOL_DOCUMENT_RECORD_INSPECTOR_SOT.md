...
**Tool ID:** `document_record_inspector`
**Type:** Standalone deterministic tool repository (no agents, no synthesis)
**Integration:** added to TechVault as a git submodule under `backend/tools/<tool_name>`; auto-discovered by TechVault via `tool.toml`.
...
Notes:
- TechVault discovers tools automatically by finding `tool.toml` under `backend/tools/<tool_name>`.
- The tool's router is mounted after manifest validation and activation determination.
...
- TechVault discovers tools automatically from `backend/tools/<tool_name>/tool.toml`.
- After manifest validation, activation is determined by local operator overrides first, then `enabled_by_default`.
- An invalid manifest is surfaced as an error; the tool is not silently skipped.
...
Example:
- `python -m document_record_inspector.cli search --query "python agents" --catalog-file catalog.json`
- Canonical invocation form is `python -m document_record_inspector.cli <command> ...`.
...
### 6.3 Canonical security levels (mandatory)

Use exactly this set across manifest, schemas, service code, DB constraints, and docs:
...
Tools MUST NOT introduce alternate vocabulary (for example `protected`).

### 6.4 Validation requirements (mandatory)

- Schema validation where appropriate.
- Service-layer validation always (before persistence).
- DB-level constraints when persistence exists.
...
- Canonical CLI invocation form is `python -m document_record_inspector.cli <command> ...`.

### 8.4 Idempotent create contract (when create flows exist)

For create operations with `idempotency_key`, service logic MUST follow this deterministic flow:

1. Lookup existing row by `idempotency_key`.
2. Attempt insert.
3. Catch `IntegrityError`.
4. Reload canonical row.
5. Return deterministic canonical result.

This pattern is required, not optional.

### 8.5 Concurrency safety (when uniqueness/idempotency applies)

- DB uniqueness constraints are required where applicable.
- Service layer must handle unique conflicts deterministically.
- Callers must not receive nondeterministic duplicate-create behavior.

### 8.6 Migration verification (DB-backed tools)

DB-backed tools must include migration verification and tests. Minimum smoke pattern:
...
### 9.1 Router
- Prefix must be resource-level only (for example `"/items"`), not platform-level
- Tags: `["tools:document_record_inspector"]`

Router declaration example:

```python
router = APIRouter(prefix="/items")
```

Platform mount owns the tool prefix. If mounted under `/v1/tools/document_record_inspector`, final paths become `/v1/tools/document_record_inspector/items`.
...
- Service tests for validation and deterministic behavior.
- Router/API tests for endpoint contract and error mapping.
- Security/authorization tests where security is enforced.
- Idempotency tests for create paths using `idempotency_key`.
- Migration tests for DB-backed tools (including upgrade/downgrade smoke).
...
- For DB-backed tools, migration verification passes (`alembic upgrade head` then `alembic downgrade base`)
- `platform tools verify <tool_name>` reports `overall_ok: true`
...
4. **API Surface Sanity** — if `api/router.py` exists (requires `fastapi`):
  - Router prefixes must be resource-level only (must not hardcode `/v1/tools/...`)
   - Router tags must include `tools:document_record_inspector`
