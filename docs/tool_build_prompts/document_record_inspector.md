# Tool Build Prompt Archive — document_record_inspector
...
Deterministic read-only inspection tool for returning the full record-level view of a single TechVault document, including:
...
- Tool is standalone and follows the canonical TechVault deterministic tool template.
- Tool is read-only.
- Tool inspects exactly one `document_id` per request.
- Tool should not add new platform abstractions.
- Tool should not duplicate existing TechVault list/search/RAG functionality.
- Tool should return a stable inspection payload suitable for debugging and operator workflows.
- `document_id` is the canonical document key from `documents.document_id`.
- `embeddings.chunk_id` joins to `chunks.id`.
- `ingest_runs.document_id` may be null in some failure cases, but initial implementation only returns rows that match the requested `document_id`.
- `include_text` defaults to false.
- `include_vectors` defaults to false.

## Expected validation gates

- manifest loads correctly
- router registered correctly
- endpoint available under `/v1/tools/document_record_inspector/...`
- OpenAPI snapshot stable
- deterministic ordering tests pass
- CLI output canonical and stable
- 404 behavior verified
- no mutation behavior introduced
- template structure preserved

## Copilot build prompt used
