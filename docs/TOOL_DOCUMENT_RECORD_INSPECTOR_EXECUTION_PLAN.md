...
Checkpoint 0:
- `python -c "import document_record_inspector"` succeeds
...
- Enforce canonical security-level validation in service logic before persistence (`public|internal|confidential|secret`)
- For create flows with `idempotency_key`, implement required deterministic flow: lookup → insert attempt → catch `IntegrityError` → reload canonical row
- Add DB uniqueness constraints and deterministic conflict handling where uniqueness/idempotency applies
...
- Router prefixes must be resource-level only (for example `APIRouter(prefix="/items")`)
- Do not hardcode platform prefixes such as `/v1/tools/...` inside tool routers

Checkpoint 3:
- `python -m document_record_inspector.cli health` returns valid JSON
- `python -m document_record_inspector.cli search --query "x" --catalog-file <path>` returns valid JSON

## Phase 4 — OpenAPI snapshot
- Implement openapi_snapshot generator + snapshot test

Checkpoint 4:
- `pytest tests/test_openapi_snapshot.py` passes
- `UPDATE_OPENAPI_SNAPSHOT=1 pytest` only when intentional snapshot changes are made
...
- Add API smoke tests
- Add required test categories when relevant: service tests, router/API tests, security tests, idempotency tests, migration tests, OpenAPI snapshot tests
- For DB-backed tools, add migration verification smoke tests (`alembic upgrade head` then `alembic downgrade base`)

Checkpoint 5:
- `pytest -q` all green

## Phase 6 — Tooling Gates (Compliance, Security + Deployment Readiness)
...
4. API surface sanity — router declares resource-level prefixes only and does not hardcode `/v1/tools/...`; tags remain correct
...
- Router import line: `from document_record_inspector.api.router import router as document_record_inspector_router` (after existing imports)
- `api_router.include_router(document_record_inspector_router, tags=["tools:document_record_inspector"])` call (before `__all__`)
